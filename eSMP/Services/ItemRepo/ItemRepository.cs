                ...
                else
                {
                    if (lot.HasValue && lat.HasValue)
                    {
                        ...
                        listItem = listItem.OrderBy(i =>
                                        6371 * 2 * Math.Atan2(Math.Sqrt(Math.Sin((la - i.Store.Address.Latitude) * (Math.PI / 180) / 2) * Math.Sin((la - i.Store.Address.Latitude) * (Math.PI / 180) / 2) + Math.Cos(i.Store.Address.Latitude * (Math.PI / 180))
                                        * Math.Cos(la * (Math.PI / 180)) * Math.Sin((lo - i.Store.Address.Longitude) * (Math.PI / 180) / 2) * Math.Sin((lo - i.Store.Address.Longitude) * (Math.PI / 180) / 2)), Math.Sqrt(1 - Math.Sin((la - i.Store.Address.Latitude) * (Math.PI / 180) / 2) * Math.Sin((la - i.Store.Address.Latitude) * (Math.PI / 180) / 2) + Math.Cos(i.Store.Address.Latitude * (Math.PI / 180))
                                        * Math.Cos(la * (Math.PI / 180)) * Math.Sin((lo - i.Store.Address.Longitude) * (Math.PI / 180) / 2) * Math.Sin((lo - i.Store.Address.Longitude) * (Math.PI / 180) / 2)))
                        ).ThenBy(i => _context.Sub_Items.Where(si => si.ItemID == i.ItemID).Min(si => si.Price));
                    }
                }
